...
{:else}
	...
	<div style="padding:34px 40px 72px;max-width:1260px;display:flex;flex-direction:column;gap:34px">
		<a href="/orgs/{organizationId}/customers" style="align-self:flex-start;font:500 13px 'Geist', sans-serif">
			← Back to customers
		</a>
		...
		<div class="card" style="padding:26px 28px;display:flex;gap:28px;align-items:center;flex-wrap:wrap">
			...
			<div style="flex:1;min-width:220px">
				<div style="font:600 22px/1.2 'Bodoni Moda', serif;letter-spacing:-.01em;color:var(--ink)">{data.customer.name}</div>
		...
		<div>
			<div style="font:600 15px/1 'Bodoni Moda', serif;margin-bottom:14px">Points earned by month</div>
				...
				<div style="margin-top:10px;display:flex;gap:14px">
					{#each data.months as m (m.label)}
						<div style="flex:1;text-align:center">
							<div class="mono" style="font:500 12px/1 'Geist Mono', monospace;color:var(--ink)">{m.total}</div>
							<div style="margin-top:5px;font:400 11px/1 'Geist', sans-serif;color:var(--text-muted)">{m.label}</div>
						</div>
					{/each}
				</div>
			</div>
		...
		{#if data.progress}
			<div>
				<div style="font:600 15px/1 'Bodoni Moda', serif;margin-bottom:14px">Progress to {data.progress.nextTierName}</div>
				<div class="card" style="padding:24px 28px;display:flex;align-items:center;gap:26px;flex-wrap:wrap">
					<div style="font:400 13px/1.5 'Geist', sans-serif;color:var(--text-muted);flex:1;min-width:200px">
		...
		<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:26px">
			<div>
				<div style="font:600 15px/1 'Bodoni Moda', serif;margin-bottom:14px">Rewards earned</div>
				{#if data.rewards.length === 0}
					<div style="font:400 13px 'Geist', sans-serif;color:var(--text-muted)">No rewards yet.</div>
				{:else}
					<div style="display:flex;flex-direction:column;gap:12px">
						{#each data.rewards as r, i (i)}
							<div class="stub" style="min-height:88px">
								<div style="flex:1;min-width:0;padding:15px 18px">
									<div style="font:600 14px/1.3 'Geist', sans-serif;color:var(--ink)">{r.name}</div>
								...
								<div class="stub-end" style="width:104px;flex:0 0 104px">
									<div style="font:500 9px/1.4 'Geist', sans-serif;letter-spacing:.1em;text-transform:uppercase;color:var(--stamp-green);text-align:center">
										Reward<br />on file
									</div>
								</div>
							</div>
			...
			<div>
				<div style="font:600 15px/1 'Bodoni Moda', serif;margin-bottom:14px">Coupons</div>
				{#if data.coupons.length === 0}
					<div style="font:400 13px 'Geist', sans-serif;color:var(--text-muted)">No coupons yet.</div>
				{:else}
						...
						{#each data.coupons as c, i (i)}
							<div style="display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 0;border-bottom:1px solid var(--line-2)">
								<div style="min-width:0">
									<div style="font:500 13px/1.3 'Geist', sans-serif;color:var(--ink)">{c.name}</div>
									<div class="mono" style="margin-top:4px;font:400 12px/1 'Geist Mono', monospace;color:var(--text-muted)">{c.code}</div>
								</div>
								<div style="display:flex;align-items:center;gap:12px">
									<span class="mono" style="font:400 12px 'Geist Mono', monospace;color:var(--text-muted)">{c.value}</span>
	...
	<Drawer bind:open={grantRewardOpen} title="Grant reward" note="Gives this customer the reward immediately, without checking its eligibility conditions.">
		{#if data.availableRewards.length === 0}
			<div style="font:400 13px 'Geist', sans-serif;color:var(--text-muted)">
	...
	<Drawer bind:open={issueCouponOpen} title="Issue coupon" note="Generates a new coupon code for this customer immediately, without checking its eligibility conditions.">
		{#if issuedCouponCode}
			<div style="display:flex;flex-direction:column;gap:10px">
				<div style="font:400 13px 'Geist', sans-serif;color:var(--text-muted)">Coupon issued — code:</div>
		...
		{:else if data.availableCoupons.length === 0}
			<div style="font:400 13px 'Geist', sans-serif;color:var(--text-muted)">No coupon types configured for this shop yet.</div>
		{:else}
