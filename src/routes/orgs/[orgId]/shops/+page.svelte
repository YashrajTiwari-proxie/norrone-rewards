				...
				<tbody>
					{#each shops.data as shop (shop._id)}
						<tr onclick={() => openShop(shop._id)} style="cursor:pointer">
							<td style="font:500 14px 'Geist', sans-serif;color:var(--ink)">{shop.name}</td>
		...
		<form id="edit-shop-form" onsubmit={submitUpdate}>
			<div style="display:flex;flex-direction:column;gap:18px">
				<div class="field">
					...
					<span style="font:400 12px 'Geist', sans-serif;color:var(--text-muted);margin-top:4px;display:block">
