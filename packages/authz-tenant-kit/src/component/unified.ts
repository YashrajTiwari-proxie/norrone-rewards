...
/**
 * Tiered O(1) permission check for a single scope (or global, when `scope`
 * is undefined). This is the original single-scope `checkPermission` body,
 * extracted unchanged so it can be reused per-link when walking a scope
 * chain (see `checkPermission` below). `tier === "none"` is the signal a
 * chain walk relies on to mean "no rule at this scope at all" (as opposed
 * to an explicit deny), so it can keep walking to a broader scope.
 */
async function checkPermissionForScope(
  ctx: { db: QueryCtx["db"] },
  args: {
    tenantId: string;
    userId: string;
...
export const checkPermission = query({
  args: {
    ...
    // Ordered most-specific → least-specific (e.g. [restaurant, organization]).
    // When provided, takes precedence over `scope`. Each link is checked with
    // the exact same tiered logic as a single-scope call; an explicit deny
    // at any link (tier !== "none") short-circuits immediately — a deny at a
    // more specific scope is never overridden by a broader allow. Otherwise
    // the first link that resolves to an allow wins. This lets one
    // organization-wide role assignment satisfy every restaurant-scoped
    // check under that org, instead of fanning out one row per restaurant.
    scopeChain: v.optional(v.array(v.object({ type: v.string(), id: v.string() }))),
  },
  returns: v.object({
    allowed: v.boolean(),
    reason: v.string(),
  ...
  handler: async (ctx, args) => {
    const chain = args.scopeChain ?? [args.scope];
    let last: PermissionCheckResult = {
      allowed: false,
      reason: "No permission granted",
      tier: "none",
    };
    for (const scope of chain) {
      const result = await checkPermissionForScope(ctx, {
        tenantId: args.tenantId,
        userId: args.userId,
        permission: args.permission,
        scope,
      });
      if (result.allowed || result.tier !== "none") {
        return result;
      }
      last = result;
    }
