/**
 * @proxie-studio/authz-tenant-kit - RBAC for multi-tenant Convex apps
 *
 * A comprehensive RBAC/ABAC/ReBAC authorization component featuring
 * O(1) indexed lookups, inspired by Google Zanzibar. Fork of
 * @djpanda/convex-authz — see NOTICE.
 *
 * @example
 * ```typescript
...
/** Validate a `scope` argument that may be a single Scope or a scope chain. */
function validateScopeOrChain(scope: Scope | Scope[] | undefined): void {
  if (Array.isArray(scope)) {
    validateScopeChain(scope);
  } else {
    validateScope(scope);
  }
}

/** The most-specific scope in a `Scope | Scope[] | undefined` argument, for error messages and deferred-policy context. */
function firstScope(scope: Scope | Scope[] | undefined): Scope | undefined {
...
// Real Convex mutation ctx objects support runQuery too — it's how a
// mutation reads from another component within the same transaction
// (e.g. the assignableRoles guard below looking up the actor's current
// roles before writing a new one). Widened from "runMutation"-only so
// that lookup doesn't need an Action round trip.
...
> {
  constructor(
    ...
    private options: {
      ...
      // Proxie fork: opt-in privilege-escalation guard for assignRole. Maps
      // each role to the set of roles a holder of that role is allowed to
      // grant to someone else. When configured, `assignRole` requires an
      // `actorId` (or `defaultActorId`), looks up that actor's current
      // roles at the target scope (or scope chain), and throws FORBIDDEN
      // unless the role being assigned is in the union of what any of the
      // actor's roles permits. Without this, holding a permission like
      // "roles:assign" would implicitly let a caller assign ANY role,
      // including one senior to their own — this closes that hole.
      assignableRoles?: Partial<Record<keyof R & string, ReadonlyArray<keyof R & string>>>;
    }
  ) {
   ...
   *
   * `scope` accepts either a single Scope (unchanged behavior) or an
   * ordered array of scopes — most-specific first — checked as a chain:
   * an explicit deny at any link short-circuits, otherwise the first link
   * that resolves to an allow wins. Pass a chain (e.g.
   * `[{type:"restaurant",id},{type:"organization",id}]`) so an org-wide
   * role assignment can satisfy a restaurant-scoped check without a
   * fanned-out row per restaurant.
   */
  ...
  async can(
    ...
    if (result.tier === "deferred" && this.options.policies) {
      return this._evaluateDeferredPolicy(ctx, userId, result.policyName, permission, firstScope(scope));
    }
    return true;
  }
  ...
  async canWithContext(
    ...
    if (!result.allowed) return false;
    if (result.tier === "deferred" && this.options.policies) {
      return this._evaluateDeferredPolicy(ctx, userId, result.policyName, permission, firstScope(scope), requestContext);
    }
    return true;
  }
  ...
  async require(
    ...
    if (!allowed) {
      const primary = firstScope(scope);
      ...
      throw new ConvexError({
        code: "FORBIDDEN",
        message: `Permission denied: ${permission}${primary ? ` on ${primary.type}:${primary.id}` : ""}`,
        permission: String(permission),
        scopeType: primary?.type ?? null,
        scopeId: primary?.id ?? null,
      });
    }
  }
  ...
  /**
   * Internal: enforce the `assignableRoles` guard (see constructor docs).
   * Fetches the actor's current roles at every link of the scope chain and
   * throws FORBIDDEN unless `targetRole` is assignable by at least one of
   * them. No-op when `assignableRoles` isn't configured.
   */
  private async _requireAssignable(
    ctx: MutationCtx | ActionCtx,
    actorId: string,
    targetRole: keyof R & string,
    scope: Scope | Scope[] | undefined,
  ): Promise<void> {
    const assignable = this.options.assignableRoles;
    if (!assignable) return;

    const chain = Array.isArray(scope) ? scope : [scope];
    const actorRoles = new Set<string>();
    for (const link of chain) {
      const scopeKey = link ? `${link.type}:${link.id}` : "global";
      const rows = await ctx.runQuery(this.component.indexed.getUserRolesFast, {
        userId: actorId,
        scopeKey,
        tenantId: this.options.tenantId,
      });
      for (const r of rows as Array<{ role: string }>) actorRoles.add(r.role);
    }

    const allowedTargets = new Set<string>();
    for (const actorRole of actorRoles) {
      const list = assignable[actorRole as keyof R & string];
      if (list) for (const t of list) allowedTargets.add(t);
    }

    if (!allowedTargets.has(targetRole)) {
      throw new ConvexError({
        code: "FORBIDDEN",
        message: `Actor's role(s) (${[...actorRoles].join(", ") || "none"}) do not permit assigning role "${targetRole}" in this scope`,
   ...
   *
   * `scope` accepts a single Scope or an ordered chain (most-specific
   * first) — see `can`'s docs. Only the first (most specific) link of a
   * chain is the scope the role is actually assigned at; the rest are
   * used solely to evaluate the `assignableRoles` guard against roles
   * the actor holds at a broader scope (e.g. an org-wide role granting
   * assignment authority inside one of that org's restaurants).
   */
  async assignRole(
    ctx: MutationCtx | ActionCtx,
    userId: string,
    ...
    const resolvedActorId = actorId ?? this.options.defaultActorId;
    if (this.options.assignableRoles) {
      if (!resolvedActorId) {
        throw new ConvexError({
          code: "FORBIDDEN",
          message: "assignableRoles is configured — assignRole requires an actorId (or defaultActorId) to authorize the assignment",
        });
      }
      await this._requireAssignable(ctx, resolvedActorId, role, scope);
    }

    const targetScope = firstScope(scope);
    ...
    const assignmentId = await ctx.runMutation(this.component.unified.assignRoleUnified, {
      tenantId: this.options.tenantId,
      userId,
      role,
      rolePermissions,
      scope: targetScope,
      expiresAt,
      assignedBy: resolvedActorId,
      enableAudit: true,
      policyClassifications: Object.keys(policyClassifications).length > 0 ? policyClassifications : undefined,
    });

    // Idempotently register the expired-role/audit-retention cleanup
    // crons on first use, so an app doesn't need its own crons.ts just
    // to keep this component's tables from growing unbounded. No-ops
    // (and never throws) if the crons component isn't installed, e.g. in
    // tests that don't register it.
    try {
      await ctx.runMutation(this.component.cronSetup.ensureCleanupCronRegistered, {});
    } catch {
      // Best-effort — a missing crons component must never fail an assignment.
    }
 ...
 * @example
 * ```ts
 * import { matchesPermissionPattern } from "@proxie-studio/authz-tenant-kit";
