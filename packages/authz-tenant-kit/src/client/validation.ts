...
/**
 * Validate a scope chain: a non-empty array of scopes (most-specific first),
 * each individually valid per `validateScope`.
 */
export function validateScopeChain(scopeChain: ScopeLike[]): void {
  if (!Array.isArray(scopeChain) || scopeChain.length === 0) {
    throw new Error("scopeChain must be a non-empty array of scopes");
  }
  for (const scope of scopeChain) {
    if (scope === undefined || scope === null) {
      throw new Error("scopeChain entries must not be undefined/null — omit `scope` entirely for a global check");
    }
    validateScope(scope);
  }
}
